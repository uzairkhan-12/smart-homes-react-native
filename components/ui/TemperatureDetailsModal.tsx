...
interface TemperatureDetailsModalProps {
  visible: boolean;
  onClose: () => void;
...
export default function TemperatureDetailsModal({
  visible,
  onClose,
  temperatureSensors,
  sensorData
}: TemperatureDetailsModalProps) {
        ...
        <View style={styles.dataContainer}>
          <View style={[styles.dataItem, isDark && styles.dataItemDark]}>
            ...
            <Text style={[styles.dataDescription, isDark && styles.textSecondaryDark]}>
              {tempValue < 18 ? 'Cold' : tempValue < 22 ? 'Cool' : tempValue < 26 ? 'Warm' : 'Hot'}
            </Text>
          </View>
        </View>
      </View>
            ...
            <View style={styles.headerLeft}>
              ...
              <View>
                ...
                <Text style={[styles.headerSubtitle, isDark && styles.textSecondaryDark]}>
                  {temperatureSensors.length} sensor{temperatureSensors.length !== 1 ? 's' : ''}
                </Text>
              </View>
            </View>
            <TouchableOpacity
          ...
          {temperatureSensors.length === 0 ? (
            <View style={styles.emptyContainer}>
              ...
              <Text style={[styles.emptyMessage, isDark && styles.textSecondaryDark]}>
                Configure temperature sensors in Settings to see readings here.
              </Text>
            </View>
          ) : (
            <>
              {/* Temperature Sensors Grid - 2 per row */}
              ...
              {temperatureSensors.every(device => !getSensorData(device)?.new_state) && (
                <View style={styles.emptyContainer}>
                  ...
                  <Text style={[styles.emptyMessage, isDark && styles.textSecondaryDark]}>
                    Check your Home Assistant connection and temperature sensor configuration.
                  </Text>
                </View>
              )}
            </>
